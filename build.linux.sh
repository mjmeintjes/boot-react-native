...
travis_fold start before_script.7
  travis_cmd npm\ install\ -g\ appium --assert --echo --timing
...
travis_fold start before_script.16
  travis_cmd run-bg\ \"cd\ ../../..\ \&\&\ boot\ inst\" --assert --echo --timing
...
travis_fold start before_script.18
  travis_cmd wait-for-bg --assert --echo --timing
...
travis_result $?
travis_cmd timeout\ 600\ ./test-reloading.sh --echo --timing
travis_result $?
